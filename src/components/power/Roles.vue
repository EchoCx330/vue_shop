...
<!--      树形控件-->
      <el-tree show-checkbox :data="rightslist" :props="treeProps" node-key="id" :default-checked-keys="defKeys" ref="treeRef"></el-tree>
      <span slot="footer" class="dialog-footer">
...
export default {
    ...
    return {
      ...
      addFormRules:{
        roleName:[{required:true,message:'请输入用户名称',trigger:'blur'},
          {min:3,max:16,message: '用户名的长度在3-16个字符之间',trigger: 'blur'}],
        roleDesc:[{required:true,message:'请输入用户描述',trigger:'blur'},
          {min:3,max:16,message: '用户描述的长度在3-16个字符之间',trigger: 'blur'}]
      },
  ...
  methods: {
    ...
    getLeafKeys(node,arr) {
      //如果当前node节点节点不包含children属性，则是三级节点
      if(!node.children) {
